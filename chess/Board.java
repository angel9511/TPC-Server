...
public class Board {
     ...
     * @throws IOException
     */
    public void checkMove(String move, String color, boolean actuallyMove) throws IOException{
    	Piece[][] fakeBoard = new Piece[8][8];
    	String newBoard = this.toString();
    	for(int ii = 0; ii < 64; ii++){
			int i = ii*2;
			String pieceColor= "";
			if(newBoard.charAt(i) == 'w')
				pieceColor = "white";
			if(newBoard.charAt(i) == 'b')
				pieceColor = "black";
			if(newBoard.charAt(i+1) == 'R')
		        fakeBoard[Math.max((int) Math.floor((double)(i)/16),0)][(i/2)%8] = new Rook(pieceColor);
			if(newBoard.charAt(i+1) == 'N')
				fakeBoard[Math.max((int) Math.floor((double)(i)/16),0)][(i/2)%8] = new Knight(pieceColor);
			if(newBoard.charAt(i+1) == 'B')
				fakeBoard[Math.max((int) Math.floor((double)(i)/16),0)][(i/2)%8] = new Bishop(pieceColor);
			if(newBoard.charAt(i+1) == 'Q')
				fakeBoard[Math.max((int) Math.floor((double)(i)/16),0)][(i/2)%8] = new Queen(pieceColor);
			if(newBoard.charAt(i+1) == 'K')
				fakeBoard[Math.max((int) Math.floor((double)(i)/16),0)][(i/2)%8] = new King(pieceColor);
			if(newBoard.charAt(i+1) == 'p')
				fakeBoard[Math.max((int) Math.floor((double)(i)/16),0)][(i/2)%8] = new Pawn(pieceColor);
			if(newBoard.charAt(i+1) == '#' || newBoard.charAt(i+1) == ' ')
				fakeBoard[Math.max((int) Math.floor((double)(i)/16),0)][(i/2)%8] = null;
				
			//System.out.println(newBoard.substring(i, i+2)+" "+(i/2)%8+" "+Math.max((int) Math.floor((double)(i)/16),0)+" "+ii);
		}
        int[] moveArray = parseInput(move);
        //System.out.println(board[moveArray[0]][moveArray[1]]);

        if(fakeBoard[moveArray[0]][moveArray[1]] == null){
            throw new IOException();
        }

        if(!fakeBoard[moveArray[0]][moveArray[1]].getColor().equals(color)){
            throw new IOException();
        }

        if(fakeBoard[moveArray[2]][moveArray[3]] != null){
            if(fakeBoard[moveArray[2]][moveArray[3]].getColor().equals(color)){
                throw new IOException();
            }
        }

        if(fakeBoard[moveArray[0]][moveArray[1]].validateMove(fakeBoard, moveArray[0], moveArray[1], moveArray[2], moveArray[3])){
            ...
            if(actuallyMove){
            	...
            	fakeBoard[moveArray[2]][moveArray[3]] = fakeBoard[moveArray[0]][moveArray[1]];
            	fakeBoard[moveArray[0]][moveArray[1]] = null;
            }

            if(fakeBoard[moveArray[2]][moveArray[3]] != null){
                if(fakeBoard[moveArray[2]][moveArray[3]].getClass().isInstance(new King("white"))){
                    if(actuallyMove){
                        ((King) fakeBoard[moveArray[2]][moveArray[3]]).hasMoved = true;
                    }
                    //This Piece is a King
                    if(((King) fakeBoard[moveArray[2]][moveArray[3]]).castled){
                        if(moveArray[3] - moveArray[1] == 2){
                        	fakeBoard[moveArray[2]][moveArray[3] - 1] = fakeBoard[moveArray[2]][moveArray[3] + 1];
                        	fakeBoard[moveArray[2]][moveArray[3] + 1] = null;
                        }else{
                        	fakeBoard[moveArray[2]][moveArray[3] + 1] = fakeBoard[moveArray[2]][moveArray[3] - 1];
                        	fakeBoard[moveArray[2]][moveArray[3] - 1] = null;
                        }
                        ((King) fakeBoard[moveArray[2]][moveArray[3]]).castled = false;
                    }
                }
            }
        ...
        if(actuallyMove){
            Piece piece = fakeBoard[moveArray[2]][moveArray[3]];
                ...
                if(piece.getClass().isInstance(new Pawn("white"))){
                    ...
                    if(piece.getColor().equals("white")){
                        if(moveArray[2] == 7){
                            ...
                            fakeBoard[moveArray[2]][moveArray[3]] = replacement;
                        }
                    }else{
                        if(moveArray[2] == 0){
                            switch(move.split(" ")[2].charAt(0)){
    ...
    public String toString(){
        String string = "";
        //System.out.println("Starting stringity");
        int fileCount = 0;
        ...
        for(Piece[] pieces: board){
            ...
            for(Piece piece: pieces){
                if(piece==null){
                   ...
                   // MainController.getInstance().printPiece(piece,rankCount,fileCount);
                }
                rankCount++;
            }
            fileCount++;
